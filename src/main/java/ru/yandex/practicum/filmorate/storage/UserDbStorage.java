...
public class UserDbStorage extends BaseStorage<User> implements UserStorage {
    ...
    private static final String FIND_ALL_FRIENDS_QUERY = "SELECT * FROM users " +
            ...
            "WHERE friend_id = ?)";
    private static final String FIND_ALL_MUTUAL_FRIENDS_QUERY = "SELECT * " +
            "FROM users " +
    ...
    private static final String FRIEND_REQUEST_EXISTS_QUERY = "SELECT 1 " +
            "FROM friends AS f " +
            "JOIN friend_request AS fr ON f.friend_request_id = fr.id " +
            "WHERE f.friend_id = ? AND f.user_id = ? AND NOT fr.status";
    private static final String FRIENDS_UPDATE_QUERY = "UPDATE friends AS f " +
            "SET f.friend_request_id = (SELECT id " +
            "FROM friend_request " +
            "WHERE status) " +
            "WHERE friend_id = ? AND user_id = ?";
    private static final String USERS_EXISTS_QUERY = "SELECT COUNT(*) " +
            "FROM users " +
            "WHERE id IN (?, ?)";
    private static final String FRIENDS_INSERT_QUERY = "INSERT INTO friends (user_id, friend_id, friend_request_id) " +
            "VALUES (?, ?, (SELECT id FROM friend_request WHERE NOT status))";
    private static final String FRIENDS_DELETE_QUERY = "DELETE FROM friends " +
